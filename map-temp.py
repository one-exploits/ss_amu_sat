...
def detect_bright_spots(image_path, center_gps, spatial_factor, threshold_value=200, min_contour_area=50):
    ...
    # Calculate the area of each bright spot (circle) in square kilometers and print it on the image
    for i, contour in enumerate(filtered_contours):
        ...
        cv2.putText(image, gps_text, (int(x) - 30, int(y) + radius + 35),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 4)  # GPS coordinates in red

        # Add the area in square pixels to the total area
    ...
    cv2.putText(image, total_area_text, (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 4)  # Cyan-colored text for total area in sq. km

    # Calculate the center coordinates of the image
    center_x, center_y = image.shape[1] // 2, image.shape[0] // 2

    # Draw a green circle at the center and annotate it with GPS
    cv2.circle(image, (center_x, center_y), 10, (0, 255, 255), 4)  # Adjusted thickness
    ...
    cv2.putText(image, center_annotation, (center_x - 100, center_y - 20),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 4)  # Adjusted font size and thickness
...
if __name__ == "__main__":
    center_gps = (40.7128, -74.0060)  # Example center GPS coordinates (New York City)
    spatial_factor = 30  # Adjust the spatial factor as needed (1 pixel = 30 meters)
